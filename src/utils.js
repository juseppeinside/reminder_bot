...
// Парсинг сообщения вида: "Текст сообщения" "12:00,16:00" 30 или «Текст сообщения» «12:00,16:00» 30
const parseMessage = (text) => {
  try {
    // Регулярное выражение для извлечения сообщения, времен и дней с поддержкой разных кавычек
    const regex = /^["""«]([^"""«»]+)["""»]\s+["""«]([^"""«»]+)["""»]\s+(\d+)$/;
    ...
    if (!match) {
      return {
        success: false,
        error:
          'Неверный формат сообщения. Используйте: "Текст сообщения" "ЧЧ:ММ,ЧЧ:ММ" КОЛ-ВО_ДНЕЙ или «Текст сообщения» «ЧЧ:ММ,ЧЧ:ММ» КОЛ-ВО_ДНЕЙ',
      };
    }
